    ...
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        {order.lineItems.map((li, i) => {
          ...
          return (
            <div key={i} className="card" style={{ display: 'flex', alignItems: 'center', gap: 12, opacity: done ? 0.7 : 1, transition: 'opacity .2s' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '.88rem', fontWeight: 600, textDecoration: done ? 'line-through' : 'none', color: done ? 'var(--success)' : 'var(--text)' }}>{li.name}</div>
                {li.qty > 1 && <div style={{ fontSize: '.72rem', color: 'var(--muted)', marginTop: 2 }}>Need {li.qty}</div>}
              </div>
              {li.qty === 1 ? (
                <button onClick={() => onPickChange(order.id, i, done ? 0 : 1)} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <button onClick={() => onPickChange(order.id, i, Math.max(0, picked - 1))} disabled={picked === 0}
                    style={{ width: 34, height: 34, borderRadius: '50%', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer', fontSize: '1.3rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text)', opacity: picked === 0 ? 0.3 : 1 }}>−</button>
                  <span style={{ minWidth: 44, textAlign: 'center', fontWeight: 700, fontSize: '.95rem', color: done ? 'var(--success)' : 'var(--text)' }}>{picked}/{li.qty}</span>
                  <button onClick={() => onPickChange(order.id, i, Math.min(li.qty, picked + 1))} disabled={picked >= li.qty}
                    style={{ width: 34, height: 34, borderRadius: '50%', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer', fontSize: '1.3rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text)', opacity: picked >= li.qty ? 0.3 : 1 }}>+</button>
                </div>
              )}
            </div>
          )
