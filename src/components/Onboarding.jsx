...
export function Onboarding({ onConnect, onDemo }) {
  ...
  const [biz, setBiz] = useState({ businessName: '', email: '', phone: '', address1: '', address2: '', city: '', postcode: '', country: '', currency: 'GBP', defaultTax: '20' })
  ...
  if (phase === 'details') return wrap(
    <div style={{ width: '100%', maxWidth: 400 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div className="field">
          <label>City</label>
          <input type="text" placeholder="London" value={biz.city} onChange={e => setBiz(b => ({ ...b, city: e.target.value }))} />
        </div>
        <div className="field">
          <label>Postcode / ZIP</label>
          <input type="text" placeholder="SW1A 1AA" value={biz.postcode} onChange={e => setBiz(b => ({ ...b, postcode: e.target.value }))} />
        </div>
      </div>
      <div className="field">
        <label>Country</label>
        <input type="text" placeholder="United Kingdom" value={biz.country} onChange={e => setBiz(b => ({ ...b, country: e.target.value }))} />
      </div>
      <div className="field">
  ...
  return wrap(
    <div style={{ width: '100%', maxWidth: 400 }}>
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, color: 'var(--accent)', marginBottom: 8 }}>All set up!</h2>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 28 }}>
        {tourPreview.map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 10 }}>
            <div style={{ width: 34, height: 34, borderRadius: 8, background: 'rgba(245,166,35,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Icon name={item.icon} />
            </div>
            <div>
      ...
      <button className="btn btn-primary btn-full" style={{ marginBottom: 10 }} onClick={() => onConnect(apiKey.trim(), fetchedProducts, biz, true)}>
        Start Tour →
      </button>
      <button className="btn btn-ghost btn-full" onClick={() => onConnect(apiKey.trim(), fetchedProducts, biz, false)}>
        Skip, go to app
      </button>
    </div>
  )
