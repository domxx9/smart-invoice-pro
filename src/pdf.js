...
export function buildInvoicePDF(inv, settings) {
  const DEFAULTS = { primaryColor: '#f5a623', secondaryColor: '#1e1e1e', tertiaryColor: '#f5f5f5', showLogo: true, showNotes: true, showTaxLine: true, showFooter: true, footerText: 'Thank you for your business.', logo: null }
  ...
  const cFmt = (n) => new Intl.NumberFormat(undefined, { style: 'currency', currency: settings.currency || 'GBP' }).format(n || 0)
  ...
  if (tmpl.showLogo && tmpl.logo) {
    try { doc.addImage(tmpl.logo, 'JPEG', margin, y, 22, 11); y += 13 }
    catch { /* skip */ }
  }
  ...
  if (inv.customerBusiness) {
    ...
    doc.text(inv.customerBusiness, colL, y, { maxWidth: colLW }); y += 5
  }
  doc.setFont('helvetica', inv.customerBusiness ? 'normal' : 'bold')
  doc.setFontSize(inv.customerBusiness ? 8 : 9); doc.setTextColor(20, 20, 20)
  doc.text(inv.customer || '—', colL, y, { maxWidth: colLW }); y += 4.5

  if (inv.email) {
    doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5); doc.setTextColor(100, 100, 100)
    doc.text(inv.email, colL, y, { maxWidth: colLW }); y += 4
  }
  const addrLines = [inv.address1, inv.address2,
    [inv.city, inv.postcode].filter(Boolean).join(', '), inv.country,
  ].filter(Boolean)
  if (addrLines.length) {
    doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5); doc.setTextColor(80, 80, 80)
    addrLines.forEach(line => { doc.text(line, colL, y, { maxWidth: colLW }); y += 4 })
  }

  // Meta right column
  ;[['Invoice', inv.id], ['Date', inv.date || '—'], ['Due', inv.due || '—'], ['Status', (inv.status || '').toUpperCase()]].forEach(([label, val], i) => {
    const my = topY + i * 5.5
  ...
  const bizAddr2 = [settings.address1, settings.address2,
    [settings.city, settings.postcode].filter(Boolean).join(', '), settings.country,
  ].filter(Boolean)
  ...
  doc.text('Subtotal', margin, y); doc.text(cFmt(sub), W - margin, y, { align: 'right' }); y += 6
  if (tmpl.showTaxLine) {
    doc.text(`Tax (${inv.tax || 0}%)`, margin, y); doc.text(cFmt(tax), W - margin, y, { align: 'right' }); y += 7
  }
  doc.setFillColor(...primary)
  doc.rect(margin, y - 3, tableW, 10, 'F')
  doc.setFont('helvetica', 'bold'); doc.setFontSize(9.5); doc.setTextColor(0, 0, 0)
  doc.text('TOTAL', margin + 4, y + 4); doc.text(cFmt(total), W - margin - 2, y + 4, { align: 'right' })
  y += 14
  ...
  if (tmpl.showNotes && inv.notes) {
    ...
    doc.text(inv.notes, margin, y, { maxWidth: tableW }); y += 12
  }
  ...
  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...secondary)
  doc.text(settings.businessName || 'My Business', margin, y)

  let brY = y
  if (settings.email) {
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(80, 80, 80)
    doc.text(settings.email, W - margin, brY, { align: 'right' }); brY += 5
  }
  if (settings.phone) {
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(80, 80, 80)
  ...
  bizAddr2.forEach(line => { doc.text(line, margin, y); y += 5 })
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    if (tmpl.showFooter && p === totalPages) {
      doc.setFont('helvetica', 'italic'); doc.setFontSize(7.5); doc.setTextColor(...primary)
    ...
    doc.text(`${inv.id}  ·  Page ${p} / ${totalPages}  ·  Generated by Smart Invoice Pro`, W / 2, H - 3, { align: 'center' })
  }
...
export async function savePDFToPhone(inv, settings, filenameOverride) {
  ...
  if (isNative) {
    try {
      ...
      const { Filesystem, Directory } = await import('@capacitor/filesystem')
      await Filesystem.writeFile({ path: filename, data: pdfBase64, directory: Directory.Documents, recursive: true })
